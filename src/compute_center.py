...
def main():
    ...
    if args.backbone == 'resnet':
        ...
        checkpoint = torch.load(args.model_path, weights_only=False)['params'] # cambiado 
        ...
        model_dict = model.state_dict()
        checkpoint = torch.load(args.model_path, weights_only=False)['params'] # cambiado por version pytorch
        checkpoint = {k: v for k, v in checkpoint.items() if k in model_dict}
    ...
    if args.dataset == 'MiniImageNet':
        trainset = ImageFolder('MINIIMAGENET/miniimagenet/train', transform=transform_val if args.backbone == 'resnet' else transform_val_224)
        ...
        trainset = ImageFolder('CIFAR-FS/cifar-fs/train', transform=transform_val_cifar if args.backbone == 'resnet' else transform_val_224_cifar)
